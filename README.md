...
> File-Sync-Tool - Open source tool by AetherCodeHQ

`cli` `file-sync` `golang` `synchronization` `utilities`

## What is it?

**File Sync Tool** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./File-Sync-Tool --path ./target

# With options
./File-Sync-Tool --path ./target --format json --output report.json

# Verbose mode
./File-Sync-Tool --path ./target --verbose
```
...
### Basic scan
```bash
./File-Sync-Tool --path ./my-project
```

### JSON report
```bash
./File-Sync-Tool --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run File Sync Tool
  run: ./File-Sync-Tool --path . --format json --output report.json
```
